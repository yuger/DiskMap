...
import com.wangxingyu.diskmap.utils.MimeTypes;
...
public class DiskUsage extends LoadableActivity {
    ...
    void continueDelete(String path) {
        ...
        if (entry != null) {
            ...
            Toast.makeText(this, "Oops. Can't find directory to be deleted.", Toast.LENGTH_SHORT).show();
        }
    }
    ...
    public void askForDeletion(final FileSystemEntry entry) {
        ...
        if (entry instanceof FileSystemEntrySmall) {
            Toast.makeText(this, "Delete directory instead", Toast.LENGTH_SHORT).show();
        ...
        if (entry.children == null || entry.children.length == 0) {
            ...
            new AlertDialog.Builder(this).setTitle(new File(fullPath).isDirectory() ? format(R.string.ask_to_delete_directory, path) : format(R.string.ask_to_delete_file, path)).setPositiveButton(str(R.string.button_delete), new DialogInterface.OnClickListener() {
                public void onClick(DialogInterface dialog, int which) {
                    BackgroundDelete.startDelete(DiskUsage.this, entry);
                }
            }).setNegativeButton(str(R.string.button_cancel), new DialogInterface.OnClickListener() {
    ...
    public boolean isIntentAvailable(Intent intent) {
        ...
        List<ResolveInfo> res = packageManager.queryIntentActivities(intent, PackageManager.MATCH_DEFAULT_ONLY);
        return res.size() > 0;
    }
...
//      }
            ...
            intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
            intent.setData(uri);
            intent.putExtra("org.openintents.extra.TITLE", str(R.string.title_in_oi_file_manager));
            intent.putExtra("org.openintents.extra.BUTTON_TEXT", str(R.string.button_text_in_oi_file_manager));
            ...
            if (isIntentAvailable(installSolidExplorer)) {
                new AlertDialog.Builder(this).setCancelable(true).setTitle("Missing compatible file manager").setMessage("No compatible filemanager found.\n\nAsk you favorite file manager developer " +
                        ...
                        "\n * Solid Explorer" +
                        "\n * OI File Manager").setPositiveButton("Install Solid Explorer", new DialogInterface.OnClickListener() {
                    @Override
                ...
                Toast.makeText(this, str(R.string.install_oi_file_manager), Toast.LENGTH_SHORT).show();
            }
            return;
        }
        ...
        Toast.makeText(this, str(R.string.no_viewer_found), Toast.LENGTH_SHORT).show();
    }
    ...
    Runnable makeProgressUpdater(final ProgressGenerator scanner, final FileSystemStats stats) {
        return new Runnable() {
    ...
    FileSystemSuperRoot scan() throws IOException, InterruptedException {
        ...
        Runnable progressUpdater = makeProgressUpdater(scanner, stats);
        handler.post(progressUpdater);

        MountPoint realMountPoint = mountPoint;
        boolean fakeDataHoneycomb = (key.equals("storage:/data"));
...
//      if (true) throw new RuntimeException("native fail");
            ...
            final Scanner legacyScanner = new Scanner(20, stats.blockSize, realMountPoint.getExcludeFilter(), stats.busyBlocks, heap);
        ...
        if (fakeDataHoneycomb) {
            FileSystemEntry media = FileSystemRoot.makeNode("media", realMountPoint.root).setChildren(entries.toArray(new FileSystemEntry[0]), stats.blockSize);
            entries = new ArrayList<>();
            entries.add(media);

            FileSystemEntry[] appList = loadApps2SD(false, AppFilter.getFilterForHoneycomb(), stats.blockSize);
            if (appList != null) {
        ...
        rootElement = FileSystemRoot.makeNode(getRootTitle(), mountPoint.root).setChildren(entries.toArray(new FileSystemEntry[entries.size()]), stats.blockSize);
    ...
    static class FileSystemStats {
        ...
        public FileSystemStats(MountPoint mountPoint) {
            ...
            if (stats != null) {
                blockSize = stats.getBlockSize();//getBlockSizeLong()
        ...
        public String formatUsageInfo() {
            ...
            return String.format("Used %s of %s", FileSystemEntry.calcSizeString(busyBlocks * blockSize), FileSystemEntry.calcSizeString(totalBlocks * blockSize));
        }
    }
    ...
    private final class GingerbreadPackageViewer extends VersionedPackageViewer {
        ...
        void viewPackage(String pkg) {
            ...
            Intent viewIntent = new Intent(Settings.ACTION_APPLICATION_DETAILS_SETTINGS, Uri.parse("package:" + pkg));
