...
public class FileSystemState {
    ...
    public FileSystemState(DiskUsage context, FileSystemSuperRoot root) {
    ...
    public void onMotion(float newTouchX, float newTouchY, long moveTime) {
        ...
        if (Math.abs(touchOffsetX) < 10 && Math.abs(touchOffsetY) < 10 && !touchMovement) return;
        touchMovement = true;

        viewDepth -= touchOffsetX / FileSystemEntry.elementWidth;
        if (viewDepth * FileSystemEntry.elementWidth < -screenWidth * 0.6) viewDepth = -screenWidth * 0.6f / FileSystemEntry.elementWidth;
    ...
    public final boolean onTouchEvent(MyMotionEvent ev) {
        try { // finally requestRepaintGPU()
            ...
            if (multitouchHandler.handleTouch(ev)) return true;
            ...
            if (action == MotionEvent.ACTION_DOWN) {
                ...
                touchX = newTouchX;
                touchY = newTouchY;
                touchDepth = (FileSystemEntry.elementWidth * viewDepth + touchX) / FileSystemEntry.elementWidth;
    ...
    public void replaceRootKeepCursor(final FileSystemSuperRoot newRoot, String searchQuery) {
        view.runInRenderThread(new Runnable() {
            @Override
            public void run() {
                FileSystemEntry oldPosition = cursor.position;
                FileSystemEntry newPosition = newRoot.getEntryByName(oldPosition.path2(), false);
    ...
    public void startZoomAnimationInRenderThread(final FileSystemSuperRoot newRoot, final boolean animate, final boolean keepCursor) {
        view.runInRenderThread(new Runnable() {
            @Override
            public void run() {
...
//      view.requestRepaint();
            ...
            if (targetViewTop < 0 || targetViewBottom > masterRoot.encodedSize || viewDepth < 0 || FileSystemEntry.elementWidth > maxElementWidth) {
                prepareMotion(SystemClock.uptimeMillis());
                animationDuration = 300;
//        view.requestRepaint();
                needRepaint = true;
    ...
    private  void paintSlowGPU(final RenderingThread rt, long viewTop, float viewDepth, int screenWidth, int screenHeight) {
    ...
    public boolean onDrawGPU(RenderingThread rt) {
        ...
        try {
            ...
            paintSlowGPU(rt, displayTop, viewDepth, screenWidth, screenHeight);
    ...
    private  void paintSlow(final Canvas canvas, long viewTop, float viewDepth, Rect bounds, int screenHeight) {
    ...
    public final void onDraw(final Canvas canvas) {
        try {
            ...
            paintSlow(canvas, displayTop, viewDepth, bounds, screenHeight);
...
//      Log.d("diskusage", "zoom false");
            fullZoom = currDepth >= prevDepth && entry.encodedSize * yscale > FileSystemEntry.fontSize * 2;
...
//      Log.d("diskusage", "zoom levels overlap, fullZoom = " + fullZoom);
    ...
    private void zoomFitLabelMoveUp(long eventTime) {
        ...
        if (targetViewBottom < cursor.top + cursor.position.encodedSize) {
            ...
            long offset = cursor.top + cursor.position.encodedSize - (long) (targetViewTop * 0.2 + targetViewBottom * 0.8);
            targetViewTop += offset;
            targetViewBottom += offset;
            if (targetViewBottom > masterRoot.encodedSize) {
    ...
    private void zoomFitToScreen(long eventTime) {
        if (targetViewTop < cursor.top && targetViewBottom > cursor.top + cursor.position.encodedSize) {
...
//    }
    ...
    private void deleteDeletingEntry() {
        ...
        if (freeSpace != null) {
            ...
            masterRoot.children[0].setSizeInBlocks(masterRoot.children[0].getSizeInBlocks() + deletingEntryBlocks, displayBlockSize);
...
//    Log.d("diskusage", "newSize = + " + newSize);
        ...
        if (freeSpace != null) {
            ...
            masterRoot.children[0].setSizeInBlocks(masterRoot.children[0].getSizeInBlocks() - dSize, displayBlockSize);
        ...
        while (true) {
            ...
            if (entry.children == null || entry.children.length == 0) return;
    ...
    public void saveState(Bundle outState) {
        ...
        outState.putInt("zoomState", zoomState == ZoomState.ZOOM_ALLOCATED ? 0 : (zoomState == ZoomState.ZOOM_FULL ? 1 : 2));
    }
    ...
    private long getFreeSpaceZoom() {
        ...
        required *= 40f / 40.5f;
        if (required < freeSpaceZoom * 0.9f) freeSpaceZoom = required;
    ...
    private void toggleZoomState() {
        zoomState = (zoomState == ZoomState.ZOOM_ALLOCATED) ? ZoomState.ZOOM_FULL : ZoomState.ZOOM_ALLOCATED;
        setZoomState();
    }
...
//         boolean post(Runnable r);
    ...
    static class MainThreadAction {
        ...
        public void warnOnFileSelect() {
            Toast.makeText(context, "Press menu to preview or delete", Toast.LENGTH_SHORT).show();
        }
    ...
    public static class VersionedMultitouchHandler {
        private static VersionedMultitouchHandler newInstance(FileSystemState view) {
    ...
    private final class MultiTouchHandler extends VersionedMultitouchHandler {
        ...
        private MotionFilter getFilterX(int i) {
            if (filterX.size() <= i) filterX.add(new MotionFilter());
            return filterX.get(i);
        }

        private MotionFilter getFilterY(int i) {
            if (filterY.size() <= i) filterY.add(new MotionFilter());
...
//          Log.d("diskusage", "multi move: reset");
                ...
                if (FileSystemEntry.elementWidth < minElementWidth) FileSystemEntry.elementWidth = minElementWidth;
