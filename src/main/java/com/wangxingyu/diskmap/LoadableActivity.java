...
public abstract class LoadableActivity extends AppCompatActivity {
    ...
    private static void handleOutOfMemory(final Activity activity) {
        try {
            ...
            new AlertDialog.Builder(activity).setTitle(activity.getString(R.string.out_of_memory)).setOnCancelListener(new OnCancelListener() {
                public void onCancel(DialogInterface dialog) {
                    activity.finish();
                }
            }).create().show();
        } catch (Throwable t) {
            Toast.makeText(activity, "DiskUsage is out of memory. Sorry.", Toast.LENGTH_SHORT).show();
        }
    }
    ...
    void LoadFiles(final LoadableActivity activity, final DiskUsage.AfterLoad runAfterLoad, boolean force) {
        boolean scanRunning;
...
//    thisLoading.setIndeterminate(true);
            ...
            public void run() {
                ...
                try {
                    ...
                    handler.post(new Runnable() {
                        public void run() {
                            ...
                            state.loading = null;
                            DiskUsage.AfterLoad afterLoadCopy = state.afterLoad;
                            state.afterLoad = null;
                ...
                handler.post(new Runnable() {
                    public void run() {
                        if (state.loading == null) return;
                        state.loading.dismiss();
                        new AlertDialog.Builder(activity).setTitle(finalError).setOnCancelListener(new OnCancelListener() {
                            public void onCancel(DialogInterface dialog) {
                                activity.finish();
                            }
                        }).create().show();
    ...
    private void handleEmptySDCard(final LoadableActivity activity, final DiskUsage.AfterLoad afterLoad) {
        new AlertDialog.Builder(activity).setTitle(activity.getString(R.string.empty_or_missing_sdcard)).setPositiveButton(activity.getString(R.string.button_rescan), new OnClickListener() {
            public void onClick(DialogInterface dialog, int which) {
                if (afterLoad == null) throw new RuntimeException("afterLoad is empty");
