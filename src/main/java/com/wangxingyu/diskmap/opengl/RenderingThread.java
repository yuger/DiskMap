...
public class RenderingThread extends AbstractRenderingThread {
    private static final float vertexData[][] = {{0.1f, 0.2f, 0}, {0.9f, 0.2f, 0}, {0.9f, 0.9f, 0}, {0.1f, 0.9f, 0}};
...
//    textBgPaint.setFlags(textPaint.getFlags() | Paint.ANTI_ALIAS_FLAG);
    ...
    public RenderingThread(Context context, FileSystemState eventHandler) {
        updateFonts(context);
        ...
        for (int i = 0; i < MAX_RECTS; i++) {
            indicies.put(new short[]{(short) (vertex), (short) (1 + vertex), (short) (2 + vertex), (short) (vertex), (short) (2 + vertex), (short) (3 + vertex)});
    ...
    public void drawVertexes(float[] out, int pos, float x0, float y0, float x1, float y1) {
        out[pos] = x0;
        out[pos + 1] = y0;
...
//    gl.glEnable(GL10.GL_DEPTH_TEST);
    ...
    static class TextPixels {
        ...
        public void draw(RenderingThread rt, float x0, float y0, int elementWidth) {
            ...
            if (bitmapMap == null) {
                ...
                canvas.save();
                canvas.clipRect(new Rect(mapX - 1, mapY, mapX + drawing + 1, mapY + textHeight));
            ...
            rt.drawVertexes(bitmapMap.vertexes, nrect * 12, x0, y0 - textBaseline, x0 + drawing, y0 + textHeight - rt.textBaseline);
    ...
    public class Square {
        ...
        public void flush() {
            ...
            gl.glDrawElements(GL10.GL_TRIANGLES, nrects * 6, GL10.GL_UNSIGNED_SHORT, indicies);
            nrects = 0;
        }
    }
...
//      black = LoadTexture(getBitmap(R.drawable.black_gradient));
        ...
        public final void drawVertexes(int pos, float x0, float y0, float xoff1, float yoff1, float xoff2, float yoff2) {
            vertexes[pos] = x0;
            vertexes[pos + 1] = y0;
...
//      gl.glBindTexture(GL10.GL_TEXTURE_2D, black);
            ...
            vertexBuffer.position(0);
            gl.glDrawElements(GL10.GL_TRIANGLES, 2 * 6, GL10.GL_UNSIGNED_SHORT, indicies);
            gl.glTexCoordPointer(2, GL10.GL_FLOAT, 0, texCoords);
    ...
    private class BitmapMap implements Comparable<BitmapMap> {
        ...
        public void edit() {
            if (editedBitmap == null) {
                editedBitmap = Bitmap.createBitmap(TEXTURE_SIZE, TEXTURE_SIZE, Bitmap.Config.ARGB_8888);
        ...
        public void flushNoDeps() {
            ...
            textTexCoords.position(0);
            vertexBuffer.position(0);
            gl.glDrawElements(GL10.GL_TRIANGLES, nrect * 6, GL10.GL_UNSIGNED_SHORT, indicies);
            nrect = 0;
            gl.glDisable(GL10.GL_BLEND);
